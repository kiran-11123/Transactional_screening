...
export async function transaction_service({ sender_customer_id, receiver_customer_id, amount, country_origin, country_destination, idempotent_key }) {
    ...
    try {
        ...
        await save_data.save({ session });
        const update_idempotent_key = await outbox_model.findOneAndUpdate({ idempotent_key }, { status: 'success' }, { new: true, session });
        ...
        await outbox_model.findOneAndUpdate({ idempotent_key }, { status: 'failed' }, { new: true, session });
        throw er;
    }
    finally {
        session.endSession();
